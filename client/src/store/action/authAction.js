...
export const loginAction = (email, password) => async(dispatch) =>{
    ...
    try {
        ...
        const {data} = await authInstance.post("/auth/login", {email, password})
...
export const signUpAction = (name, email, password) => async(dispatch)=>{
    try {
        ...
        const {data} = await authInstance.post("/auth/register", {name, email, password})
...
export const changePassAction = (currentPassword, newPassword) => async(dispatch) => {
    try {
        dispatch({type: "changePassReq"})
        
        const {data} = await authInstance.put("/auth/password/change", {currentPassword, newPassword})
...
export const forgetPassAction = (email) => async(dispatch) => {
    try {
        ...
        const {data} = await authInstance.post("/auth/password/forget", {email})
...
export const resetPassAction = (newPassword, confirmPassword, token) => async(dispatch) => {
    try {
        dispatch({type: "resetPassReq"})
        
        const {data} = await authInstance.put("/auth/password/reset/"+token, {newPassword, confirmPassword})
